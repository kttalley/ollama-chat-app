  ...
  return (
    <div className="flex-1 overflow-y-auto p-4 space-y-3 mx-[1rem] pb-[2rem]">
      {displayMessages.length === 0 ? (
        <div className="flex flex-col justify-center items-center mt-[240px] h-full text-center text-gray-600">
          <Icon name="sparkle-bold" className="scale-400" />
          <h3 className="mt-[48px] text-lg font-semibold text-gray-900">Hello, I'm a locally hosted AI Assistant</h3>
        ...
        <>
          ...
          {displayMessages.map((msg, idx) => {
            ...
            return (
              <div key={idx} className={`flex flex-col ${isUser ? '' : 'items-start'}`}>
                <div
                  className={`max-w-xs px-3 py-2 rounded-lg text-sm leading-relaxed break-words ${
                    isUser
                      ? 'bg-[#ffffff00] my-[1rem] m-[0.25rem] py-[0.5rem] rounded-[1rem]'
                      : 'text-[var(--mds-color-theme-secondary-light)] rounded-[1rem] bg-[#ffffff44]'
                  }`}
                  style={{
                ...
                >
                  {isUser ? (
                    <div className="prose prose-sm max-w-[96%]">
                    ...
                    <div className="prose prose-sm max-w-[96%]">
                      <div className="flex flex-row justify-between">
                        <div className="flex flex-row items-center gap-[0.5rem]">
                          <Icon name="sparkle-filled" className="scale-200 mr-[4px]" />
                          <Text type="body-large-bold">LLM Agent</Text>
                        </div>
                        <Text type="body-small-regular">{timestamp.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</Text>
                      </div>
                      <ReactMarkdown>{msg.content}</ReactMarkdown>
